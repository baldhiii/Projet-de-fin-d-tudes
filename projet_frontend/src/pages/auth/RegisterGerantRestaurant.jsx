...
export default function RegisterGerantRestaurant() {
  ...
  const handleSubmit = async (e) => {
    ...
    setError("");
    try {
      await axios.post("http://localhost:8000/api/auth/register-gerant-restaurant/", form);
      navigate("/login-gerant-restaurant");
    } catch (err) {
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-r from-green-700 to-emerald-800">
      <div className="bg-white rounded-xl shadow-lg w-full max-w-5xl flex overflow-hidden">
        ...
        >
          <h2 className="text-2xl font-bold text-emerald-700 mb-6">INSCRIPTION GÉRANT RESTAURANT</h2>
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <button
              type="submit"
              className="w-full py-2 bg-gradient-to-r from-emerald-500 to-green-600 text-white font-semibold rounded-md hover:opacity-90 transition"
            >
              S’INSCRIRE
            </button>
          </form>
        ...
        <motion.div
          initial={{ x: 50, opacity: 0 }}
          animate={{ x: 0, opacity: 1 }}
          className="w-1/2 bg-gradient-to-br from-green-800 to-emerald-900 text-white p-10 flex flex-col justify-center items-center"
        >
          <h1 className="text-3xl font-bold mb-4">Bienvenue sur Luxvia</h1>
          <p className="text-center text-sm max-w-sm mb-6">
            Créez un compte pour gérer votre restaurant, vos tables et vos réservations.
          </p>
          <button
            onClick={() => navigate("/login-gerant-restaurant")}
            className="px-6 py-2 bg-white text-green-800 font-semibold rounded-full hover:bg-gray-200 transition"
          >
