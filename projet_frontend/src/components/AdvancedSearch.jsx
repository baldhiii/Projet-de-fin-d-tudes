...
const villesMaroc = [
  "Casablanca", "Marrakech", "Agadir", "Fès", "Rabat",
  "Tanger", "Essaouira", "Ouarzazate", "Dakhla", "Chefchaouen"
];
  ...
  return (
    <div className="w-full flex justify-center mt-12">
      <div className="grid grid-cols-5 gap-2 items-center bg-white/80 backdrop-blur-md rounded-3xl shadow-xl px-6 py-4 max-w-5xl w-full">
        ...
        {/* Toggle Hotels / Restaurants */}
        <div className="col-span-5 flex justify-center mb-2">
          <div className="inline-flex rounded-full bg-gray-200 p-1">
            <button
              onClick={() => setType("hotel")}
              className={`px-4 py-1 rounded-full text-sm font-semibold transition ${type === "hotel" ? "bg-cyan-500 text-white" : "text-gray-700"}`}
            >
              Hôtels
            </button>
            <button
              onClick={() => setType("restaurant")}
              className={`px-4 py-1 rounded-full text-sm font-semibold transition ${type === "restaurant" ? "bg-cyan-500 text-white" : "text-gray-700"}`}
            >
              Restaurants
            </button>
          </div>
        ...
        <div ref={destinationRef} className="relative col-span-2">
          <button
            ...
            className="flex items-center w-full px-4 py-2 bg-white rounded-xl border border-gray-300 shadow-sm hover:border-cyan-400 transition"
          >
          ...
          <AnimatePresence>
            {showDestinations && (
              <motion.ul
                ...
                exit={{ opacity: 0, y: 20 }}
                className="absolute z-10 mt-2 w-full bg-white shadow-lg rounded-xl p-2 border border-gray-200 max-h-64 overflow-y-auto"
              >
                {(type === "hotel" ? villesMaroc : restaurants.map(r => r.nom)).map((item) => (
                  <li
                    key={item}
                    className="px-3 py-2 text-gray-800 hover:bg-cyan-100 rounded-md font-medium cursor-pointer transition"
                    onClick={() => {
                      setDestination(item);
        ...
        <div ref={calendarRef} className="relative col-span-2">
          ...
          className="absolute z-10 mt-2 bg-white rounded-xl shadow-lg p-3 overflow-hidden"
          style={{ height: "280px", maxHeight: "280px" }}  // ✅ hauteur fixe
        >
          ...
          <AnimatePresence>
            {showGuests && (
              <motion.div
                ...
                animate={{ opacity: 1, scale: 1 }}
                exit={{ opacity: 0, scale: 0.95 }}
                className="absolute z-10 mt-2 bg-white rounded-xl shadow-lg p-4 space-y-4"
              >
                <div>
                ...
                <div>
                  ...
                  <div className="flex items-center space-x-3">
                    <button onClick={() => type === "hotel"
                      ? setChambres((c) => Math.max(1, c - 1))
                      : setTables((t) => Math.max(1, t - 1))} className="bg-gray-200 px-3 rounded-full text-lg">−</button>
                    <span className="font-semibold">{type === "hotel" ? chambres : tables}</span>
                    <button onClick={() => type === "hotel"
                      ? setChambres((c) => c + 1)
                      : setTables((t) => t + 1)} className="bg-gray-200 px-3 rounded-full text-lg">+</button>
                  </div>
                </div>
              </motion.div>
            )}
        ...
        <div className="col-span-5 text-center mt-2">
          <button className="bg-gradient-to-r from-blue-500 to-blue-700 text-white font-semibold px-8 py-2 rounded-full hover:opacity-90 transition">
            Rechercher
          </button>
        </div>
      </div>
